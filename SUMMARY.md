# Summary

## Introduction

  * [What is this?](README.md)

## Containers

  * [Run podman on MacOS](content/mac_podman.md)
  * [Run OpenShift CLI on Alpine Linux](content/alpine_oc_cli.md)
  * [Cloud Native development with Mutagen.io](content/mutagen_dev.md)

## Kubernetes

  * [Accessing k8s with a service account](content/cluster_access_via_sa.md)
  * [IKS `fsck` errors](content/iks_fsck.md)
  * [Extract `token-id` from KUBECONFIG](content/extract_tokenid.md)
  * [Debugging IKS kubelet errors](content/iks_kubelet_errors.md)
  * [Manually creating service endpoints](content/manual_se.md)
  * [Run privileged deployments on microk8s](content/priv_mk8s.md)
  * [Sample TLS Ingress](content/sample_ingress.md)
  * [Show everying in a namespace](content/show_all.md)
  * [Access a service via kube proxy](content/svc_proxy.md)
  * [Taint k3s masters for no user workloads](content/k3s_taint.md)
  * [Minikube on CentOS KVM](content/minkube_on_centos.md)
  * [Calico/Bird status commands](content/calico_bird.md)

## OpenShift

  * [Debugging the OpenShift 4.x Router](content/ocp_router_debug.md)
  * [Managing Templates](content/ocp_templates.md)
  * [Example ClusterLogging and LogForwarding APIs](content/ocp_min_clo.md)
  * [Finding local storage devices](content/ocp_block_storage.md)
  * [Using a PVC for internal image registry](content/ocp_image_registry.md)
  * [Find OAuth endpoint for IBM Cloud OpenShift cluster](content/roks_oauth.md)
  * [Cloud Native Toolkit on CRC](content/cnt_on_crc.md)

## Istio

  * [Cleaning up dangling webhooks](content/istio_danglers.md)
  * [IBM Cloud Managed Istio upgrade test script](content/ibmcloud_managed_istio_update.md)
  * [Manual Istio cleanup](content/manual_istio_cleanup.md)
  * [Debugging Istio sidecars](content/istio_sidecars.md)

## IBM Cloud

  * [Force VPC default storage class off](content/ibmcloud_force_csi.md)
  * [Add new offerings to a private catalog instance](content/ibmcloud_private_catalog_new_offering.md)
  * [Upload to COS via curl](content/cos_upload.md)  

## Linux

  * [Install Cockpit on RHEL](content/cockpit_rhel.md)
  * [Install Cockpit on Fedora CoreOS](content/cockpit_fcos.md)  
  * [Add time to rightprompt of robbyrussell.zsh-theme](content/righttime_zsh.md)
  * [Basic IP forwarding with IPTables](content/ipfwd_iptables.md)
  * [SSH Tunneling](content/ssh_tunnels.md)
  * [Expose Unix socket over TCP](content/unix_socket_tcp.md)
  * [Always print newline after curl output](content/curl_newline.md)

## Mac

  * [Remove dangling apps in LaunchPad](content/launchpad_dangle.md)
  * [Mount sshfs file system on Mac](content/sshfs_mount.md)
  * [Use Touch ID for sudo](content/sudo_touch.md)

## TLS / SSL / GPG

  * [Extract cert from TLS host](content/cert_extract.md)
  * [Verify cert with OpenSSL](content/cert_validation.md)  
  * [Certbot manual DNS challenge](content/certbot_dns.md)
  * [1 day GPG timeout](content/gpg_timeout.md)

## Misc

  * [Enterprise authentication with Travis CLI](content/travis_enterprise.md)

---

* [Gists](https://gist.github.com/jpapejr)
* [Repos](https://github.com/jpapejr?tab=repositories)
* [Sample Resources](RESOURCES.md)


